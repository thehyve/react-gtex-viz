...
function parseModelExons(json, isFull) {
...
function parseJunctions(json) {
  ...
  if (!json.hasOwnProperty(attr)) throw 'Parsing Error: parseJunctions input error. ' + attr; // check required json attributes
...
function parseTranscripts(json) {
  ...
  if (!json.hasOwnProperty(attr)) throw 'parseIsoforms input error'; // check required attributes, use the first transcript as the test case
...
function parseExonExpression(data, exons) {
  ...
  const exonObjects = data[attr]; // error-checking
...
function parseJunctionExpression(data) {
  ...
  const junctions = data[attr]; // error-checking
...
function parseTranscriptExpression(data) {
  ...
  if (!data.hasOwnProperty(attr)) throw 'Parsing Error: parseTranscriptExpression input error'; // parse GTEx isoform median TPM
...
function createSvg(id, width, height, margin, svgId = undefined, position = "relative") {
  ...
  return select("#" + id).append("svg").attr("width", width).attr("height", height).attr("id", svgId) // .style("position", position)
    ...
    try {
      ...
      for (var j = 0; j < rules.length; j++) {
        ...
        if (typeof rule.style != "undefined") {
          var elems; // removing any selector text including svg element ID -- dom already selects for that

          var selector = rule.selectorText === undefined ? rule.selectorText : rule.selectorText.replace(`#${dom[0].id} `, ''); //Some selectors won't work, and most of these don't matter.
...
class Dendrogram {
  ...
  _drawHTree(dom) {
    ...
    dom.selectAll('.branch').data(nodes).enter().append("line").attr("x1", d => d.x).attr("x2", d => d.data.length ? d.x - this.xScale(d.data.length) : d.x).attr("y1", d => d.y + this.yScale.bandwidth() / 2).attr("y2", d => d.y + this.yScale.bandwidth() / 2).attr("stroke", "gray").attr("stroke-width", 1); // for all internal nodes
    ...
    dom.append("g").attr("class", "dendrogram-axis").attr("transform", "translate(0," + this.height + ")").call(axisBottom(this.xScale).ticks(3) // .tickValues([Math.floor(this._getMaxBranchLength()/2), Math.floor(this._getMaxBranchLength())])
    );
  }
  ...
  _drawVTree(dom) {
    ...
    dom.selectAll('.branch').data(nodes).enter().append("line").attr("y1", d => d.y).attr("y2", d => d.data.length ? d.y - this.yScale(d.data.length) : d.y).attr("x1", d => d.x + this.xScale.bandwidth() / 2).attr("x2", d => d.x + this.xScale.bandwidth() / 2).attr("stroke", "gray").attr("stroke-width", 1); // for all internal nodes
    ...
    dom.append("g") // .attr("transform", `translate(${this.width}, 0)`)
    .attr("class", "dendrogram-axis").call(axisLeft(this.yScale).ticks(3) // .tickValues([Math.floor(this._getMaxBranchLength()/2), Math.floor(this._getMaxBranchLength())])
    );
  }
  ...
  _setXScale() {
    if ('h' == this.orientation) {
      this.xScale = linear$1().domain([0, this._getMaxBranchLength()]).range([0, this.width]);
    } else {
  ...
  _setYScale() {
    if ('h' == this.orientation) {
      ...
      this.yScale = linear$1().domain([0, this._getMaxBranchLength()]).range([0, this.height]);
    }
  }
...
Dispatch.prototype = dispatch.prototype = {
  ...
  on: function(typename, callback) {
    ...
    while (++i < n) {
      if (t = (typename = T[i]).type) _[t] = set$1$1(_[t], typename.name, callback);
      else if (callback == null) for (t in _) _[t] = set$1$1(_[t], typename.name, null);
    }
...
class Toolbar {
  ...
  createButton(id, icon = 'fa-download') {
    const $button = jquery('<a/>').attr('id', id).addClass('btn btn-default').appendTo(this.bar);
  ...
  downloadSvg(svgId, fileName, cloneId) {
    ...
    let $svgCopy = svgObj.clone().attr("version", "1.1").attr("xmlns", "http://www.w3.org/2000/svg"); // parse and add all the CSS styling used by the SVG
    ...
    FileSaver_min(svgBlob, fileName); // clear the temp download div
...
class Tooltip {
  ...
  move(x = event.pageX, y = event.pageY) {
    ...
    const t = select('#' + this.id).style("left", `${x}px`).style("top", `${y}px`);
  }
...
class Heatmap {
  ...
  constructor(data, useLog = true, logBase = 10, colorScheme = "YlGnBu", r = 2, tooltipId = "heatmapTooltip") {
    ...
    this.yScale = undefined;
    this.r = r;
    this.colorScheme = colorScheme; // peripheral features
    /// Tooltip
  ...
  */
  ...
  draw(dom, dimensions = {
    ...
    const xLabels = dom.selectAll(".exp-map-xlabel").data(this.xList); // update and transform
    ...
    xLabels.enter().append("text").attr("class", (d, i) => `exp-map-xlabel x${i}`).attr("x", 0).attr("y", 0).attr("text-anchor", "start").style("cursor", "default").attr("transform", d => {
      let x = this.xScale(d) + adjust;
      let y = Y;
      return `translate(${x}, ${y}) rotate(${angle})`;
    }).merge(xLabels).text(d => d); // exit -- removes old elements as needed

    xLabels.exit().remove();
    const yLabels = dom.selectAll(".exp-map-ylabel").data(this.yList).enter().append("text").text(d => d).attr("x", this.xScale.range()[1] + 5).attr("y", d => this.yScale(d) + 10).attr("class", (d, i) => `exp-map-ylabel y${i}`).attr("text-anchor", "start").style("cursor", "default").on('click', d => {
    ...
    const cells = dom.selectAll(".exp-map-cell").data(this.data, d => d.value); // update old elements
    ...
    .attr("col", d => `y${this.yList.indexOf(d.y)}`); // enter new elements
...
class DendroHeatmap {
  ...
  constructor(columnTree, rowTree, heatmapData, color = "YlGnBu", r = 2, config = new DendroHeatmapConfig(), tooltipId = "dmapTooltip", useLog = true, base = 10, title = '') {
    this.config = config.get(); //input evaluations

    columnTree = columnTree === undefined || columnTree.startsWith("Not enough data") ? undefined : columnTree;
    rowTree = rowTree === undefined || rowTree.startsWith("Not enough data") ? undefined : rowTree; // assign attribute values based on input arguments
  ...
  _updateConfig(legendPos) {
    const rows = this.objects.rowTree === undefined ? 1 : this.objects.rowTree.leaves.length; // updates the left panel's height based on the data
...
class GeneModel {
  ...
  constructor(gene, exons, exonsCurated, junctions, isIsoform = false, maxIntronLength = 1000, minExonWidth = 0) {
    ...
    this.maxIntronLength = maxIntronLength; // hard-coded for now
  ...
  render(dom, config) {
    ...
    if (!this.isIsoform) {
      ...
      this.junctions.sort((a, b) => {
        ...
        if (+a.chromStart < +b.chromStart) return -1;
        if (+a.chromStart > +b.chromStart) return 1; // then sort by chromEnd:
      ...
      this.junctions.forEach((d, i) => {
        // d.startExon = this._findExon(d.chromStart);
        // d.endExon = this._findExon(d.chromEnd);
        d.displayName = `Junction ${i + 1}`; // d.displayName = `Exon ${d.startExon.exonNumber} - ${d.endExon.exonNumber}`;
      ...
      const juncDots = dom.selectAll(".junc").data(this.junctions); // updating elements
      ...
      const exonRects = dom.selectAll(".exon").data(this.exons); // updating elements

      exonRects.attr("x", d => d.x);
      exonRects.attr("y", exonY); // entering new elements
      ...
      .attr("x", d => d.x).merge(exonRects).style("cursor", "default"); // model info text label
      ...
      const intronLine = dom.append("line").attr("x1", this.exonsCurated[0].x).attr("x2", this.exonsCurated[this.exonsCurated.length - 1].x).attr("y1", exonY + 15 / 2).attr("y2", exonY + 15 / 2).classed("intron", true);
    }
    /***** rendering curated exons on the final gene model or isoform exons */


    const exonRects2 = dom.selectAll(".exon-curated").data(this.exonsCurated); // updating elements

    exonRects2.attr("x", d => d.x);
    exonRects2.attr("y", exonY); // entering new elements
  ...
  setXscale(w) {
    ...
    const domain = [0, sum];
    const range = [0, w];
    this.xScale = linear$1().domain(domain).range(range);
  }
  ...
  setXscaleFixIntron(w) {
    ...
    const range = [0, w];
    this.xScale = linear$1().domain(domain).range(range); // fixed intron width
...
class IsoformTrackViewer {
  ...
  showData(data, colorScale, barScale, dataLabel, sort = true) {
    ...
    g.append("circle").attr("cx", 0).attr("cy", d => this.yScale(d.transcriptId)).attr("r", 5).style("fill", d => d.value == 0 ? this.nullColor : colorScale(d.value)).transition().duration(1000).attr("cx", d => barScale(d.value)); // add the axes
    ...
    lollipopGraph.append("text").attr("id", "lolliLabel").attr("x", 0).attr("y", -40).attr("text-anchor", "end").style("font-size", 9).text("log10(TPM)"); // TODO: this should be a user-defined text
  ...
  render(redraw = false, dom = undefined, labelOn = 'left', duration = 1000) {
    ...
    const isoTracks = dom.selectAll(".isotrack").data(this.isoforms.map(d => d.transcriptId)); // update old isoform tracks, if any
  ...
  .then(function (data) {
    ...
    Promise.all(promises).then(function (args) {
      const tissues = parseTissues(args[0]),
            exons = parseModelExons(args[1], true),
            // exons of the full gene model
      exonsCurated = parseModelExons(args[2], false),
      ...
      isoformColorScale = setColorScale(isoformExpress.map(d => Math.log10(d.value + 1)), "Purples", 0); // heat map
      ...
      ["toolbar", "clone"].forEach(key => {
        jquery('<div/>').attr("id", ids[key]).appendTo(jquery(`#${ids.root}`));
      });
      const svgTitle = `${gene.geneSymbol}: ${gene.gencodeId} ${gene.description}`;
      const width = jquery(`#${rootId}`).innerWidth() || window.innerWidth;
      ...
      switch (type) {
        case "isoformTransposed":
          {
            const dmapConfig = new DendroHeatmapConfig(width, 150, 100, {
              ...
              bottom: 200,
              left: 50
            }, 12, 10); // TODO: move cluster data parsing to gtexDataParser.js
        ...
        case "junction":
          {
            if (junctions.length === 0) {
              jquery(`#${rootId}`).text('This gene has no junctions available.');
              break;
            }
      ...
      jquery('#spinner').hide(); // TODO: code review
      ...
      isoformTrackViewer.render(false, trackViewerG, labelOn); // customization
...
function _customizeHeatMap(tissues, geneModel, dmap, isoTrackViewer, junctionScale, exonScale, isoformScale, junctionData, exonData, isoformData) {
  ...
  mapSvg.selectAll(".exp-map-ylabel").on("mouseover", function () {
    ...
    geneModel.addData(mapSvg.select("#geneModel"), j, ex, junctionScale, exonScale); // isoforms update

    const isoBarScale = linear$1().domain([min(isoformData.map(d => d.value)), max(isoformData.map(d => d.value))]).range([0, -100]);
...
function _customizeIsoformTransposedMap(tissues, dmap, isoTrackViewer, junctionScale, exonScale, isoformScale, junctionData, exonData, isoformData) {
  ...
  const tooltip = dmap.tooltip; //replace tissue site detail ID with tissue site detail

  mapSvg.selectAll(".exp-map-xlabel").text(d => tissueDict[d] !== undefined ? tissueDict[d].tissueSiteDetail : d).style("cursor", "pointer"); // add tissue bands
  ...
  mapSvg.selectAll(".exp-map-ylabel").on("mouseover", function (d) {
    select(this).classed("highlighted", true); // highlight the isoform track
  ...
  }).on("click", function () {// no action implemented
  });
}
/**
...
function _customizeExonMap(tissues, geneModel, dmap) {
  ...
  mapSvg.selectAll(".exp-map-xlabel").each(function (d) {
    ...
    select(this).classed("highlighted", true); // highlight the exon on the gene model
...
function _customizeJunctionMap(tissues, geneModel, dmap) {
  ...
  mapSvg.selectAll(".exp-map-xlabel").each(function () {
    ...
    xlabel.attr("id", `${jId}`);
    xlabel.classed(`junc${jId}`, true); // and then change the text to startExon-endExon format
    ...
    select(this).classed("highlighted", true); // highlight the junction and its exons on the gene model
...
var TranscriptBrowserComponent = /*#__PURE__*/function (_Component) {
  _inherits(TranscriptBrowserComponent, _Component);

  var _super = _createSuper(TranscriptBrowserComponent);

  function TranscriptBrowserComponent() {
    var _this;

    _classCallCheck(this, TranscriptBrowserComponent);
  ...
  _createClass(TranscriptBrowserComponent, [{
    ...
    value: function render() {
      ...
      return /*#__PURE__*/React__default.createElement(React__default.Fragment, null, /*#__PURE__*/React__default.createElement("h3", null, "Transcript browser"), /*#__PURE__*/React__default.createElement("div", {
  ...
  return TranscriptBrowserComponent;
}(React.Component);
...
exports.TranscriptBrowserComponent = TranscriptBrowserComponent;
